<script lang="ts">
    let status = "CLOSED";
</script>

<div class="min-h-screen flex flex-col items-center w-5/12 mx-auto">
    <h1 class="text-4xl font-bold mt-36">Commission Info</h1>

    <h2 class="text-3xl mt-8">Commission Status: {status}</h2>

    <hr class="border-t-8 border-[#332e38] w-full mt-8 mb-8" />

    <!-- TOS -->
    <h2 class="text-3xl">TERMS OF SERVICE: (WIP)</h2>
    <ul class="list-disc text-lg">
        <li>All commissions are for personal use only. You may not profit from or use the commissioned artwork commercially unless discussed and agreed otherwise.</li>
        <li>Payment is accepted with Paypal in USD currency.</li>
        <li>Once payment is received, no refunds can be issued.</li>
        <li>Commissions are not first-come, first-served basis. I will choose the project to work on after reviewing all requests submitted.</li>
        <li>If you upload, share, or use the artwork anywhere, please give proper credit.</li>
        <li>Please refrain from asking me to mimic another artist's style.</li>
        <li>I can only work create simple backgrounds; refer to my previous works as reference.</li>
        <li>Once payment is made and a draft/sketch is approved, the process may take 1-3 weeks to complete. however, this duration may vary based on the specifics of your request.</li>
        <li>Upon completion, you will receive the final artwork in PNG format along with the original CLIP or PSD file.</li>
        <li>By submitting a request you confirm that you have read and agree to the Terms and Service above.</li>
    </ul>

    <hr class="border-t-8 border-[#332e38] w-full mt-8 mb-8" />

    <div class="flex space-x-8">
        <!-- Will Do -->
        <div class="flex flex-col w-1/2">
            <h2 class="text-3xl">Will Do:</h2>
            <ul class="text-lg">
                <li>Anime Girls!!! Character Fanart, Kemonomimi, Slightly Suggestive</li>
            </ul>
        </div>

        <!-- Won't Do -->
        <div class="flex flex-col w-1/2">
            <h2 class="text-3xl">Won't Do:</h2>
            <ul class="text-lg">
                <li>Create & Design an OC/Vtuber, Furries/Scalies, R-18 (and themes), Hateful Imagery, Mechas, Vehicles/Machinery, Armor, Firearms</li>
            </ul>
        </div>
    </div>

    <hr class="border-t-8 border-[#332e38] w-full mt-8 mb-8" />

    <h2 class="text-3xl">COMMISSION TYPES: (WIP)</h2>
    <ul class="text-lg">
        <li>This isn't final yet. I'm just adding these for space.</li>
        <li>Background is $10-$30 depending on complexity</li>
    </ul>
    
    <!-- Bust up -->
    <h3 class="text-2xl text-center mt-4">Bust Up: $69</h3>
    <img src="https://pbs.twimg.com/media/GgQTLVHXMAANF75?format=jpg" 
        alt="Raki New Years in Kimono" class="w-10/12 h-auto p-2"/>
    <img src="https://pbs.twimg.com/media/GcJjIL_WwAAB2ee?format=jpg" 
        alt="Raki's Pocky Day (Shoujo)" class="w-6/12 h-auto p-2"/>


    <!-- Full body -->
    <h3 class="text-2xl text-center mt-4">Full Body: $420</h3>
    <div class="flex justify-between w-full">
        <img 
            src="https://pbs.twimg.com/media/GdxKjJJXIAATqGr?format=jpg" 
            alt="M0MORI Debut Art" 
            class="w-1/2 p-2"
        />
        <img 
            src="https://pbs.twimg.com/media/GePtI60XQAAsAUl?format=jpg" 
            alt="Ayu 2.0 Debut Art" 
            class="w-1/2 p-2"
        />
    </div>

    <hr class="border-t-8 border-[#332e38] w-full mt-8 mb-8" />

    <p class="text-2xl">10% off</p>
    <p class="text-2xl">If you commission me to draw one of my oshis</p>
    <p>(This isn't a joke and I'm not saying who they are.)</p>

    <hr class="border-t-8 border-[#332e38] w-full mt-8 mb-8" />

    <!-- Request Form -->
    <h2 class="text-3xl">REQUEST FORM</h2>
    <ul class="text-lg">
        <li><strong>About You:</strong> give your username or twitter handle, and means of communication.</li>
        <li><strong>Paypal Email:</strong></li>
        <li><strong>Commission Type:</strong> negotiable</li>
        <li><strong>Character Details:</strong> name, image references.</li>
        <li><strong>(optional) Additional Details:</strong> outfit references, pose references, facial expressions, other details (references help a lot).</li>
    </ul>

    <hr class="border-t-8 border-[#332e38] w-full mt-8 mb-8" />

    <h2 class="text-3xl">CONTACT</h2>
    <h3 class="text-xl my-2">
        <strong class="text-[#fc6e83;]">X / Twitter: </strong>
        <a href="https://x.com/Jellis2k" class="underline">@Jellis2k</a>
    </h3>
    <h3 class="text-xl my-2">
        <strong class="text-[#fc6e83;]">Email: </strong>
        <a href="https://x.com/Jellis2k" class="">jellisvt.official@gmail.com</a>
    </h3>
    <p class="my-2">Feel free to contact through Twitter/X DMs or Email for questions.</p>

</div> 
<!-- End of content -->
 
<style>
    h1, h2 {
        color: #fc6e83;
    }
</style>